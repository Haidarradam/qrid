...
/**
 * Generate and download as scalable vector graphic (SVG).
 * SVG renders perfectly at any size — ideal for print and design work.
 */
export async function downloadAsSVG(config: QRConfig): Promise<void> {
  const QRCodeStyling = await loadQRLib();
  // SVG type renders the QR as native SVG instead of canvas
  const opts = { ...buildOptions(config, 1024), type: "svg" as const };
